...
- **通用 agent 兼容**：项目默认按 Codex / Claude Code / OpenClaw 都可阅读和执行的方式组织；LLM 配置统一在 [`core/llm_runtime.py`](../core/llm_runtime.py)，`web_search` / `web_fetch` 属于运行时可选能力，后端不支持 server-side tools 时会自动剔除或降级为客户端工具。
