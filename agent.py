...
def tool_crawl_product_specs(
    ...
    return json.dumps({
        ...
        "instruction": (
            ...
            "bom_source 标注为 'web'。FCC 检索为独立模块，本流程跳过。"
        ),
    }, ensure_ascii=False, indent=2)
...
def tool_dfma_analysis(
    ...
    """
    DFMA 功能-成本矩阵分析。
    基于 8 桶 BOM 数据 + user_value_weight 计算每桶的价值/成本比，
...
CLIENT_TOOLS: list[dict] = [
    ...
    },
    {
        "name": "dfma_analysis",
        "description": (
            "DFMA 功能-成本矩阵分析。基于 8 桶 BOM 数据与用户价值权重，计算每个子系统的价值/成本比，"
...
> 职责边界：`/bom` 只输出"是什么成本"——8 桶占比、BOM/MSRP、供应商、专利风险。
...
价格查表：`data/lib/components_lib.csv`（权威）→ `data/lib/standard_parts.json`（fallback）。
...
### Step 2 — 功能-成本矩阵
调用 `dfma_analysis`，输出 8 桶的功能-成本矩阵：
...
[dim]/bom 出成本 · /dfma 出降本 · /fcc 拆机数据采集[/dim]

[bold]核心命令[/bold]：
  • [bold cyan]/bom[/bold cyan]  石头 G30S Pro                   8 桶占比 + 供应链/风险
