...
def tool_generate_bom_estimate(
    ...
    """
    按7桶结构生成 BOM 成本预估表，并写回 bom_cost 字段。
    overrides 可传入已知成本，格式同 bom_cost 字段名。
    如 {"perception_control_cny": 450, "battery_bms_cny": 160}
    """
    db = load_db()
    if product_key not in db:
        return json.dumps({"error": f"产品 '{product_key}' 不存在"}, ensure_ascii=False)
    ...
    # 行业基准占比（中点值）
    BUCKETS = [
        ("perception_control_cny", "感知与控制",   0.18, "主板+摄像头+LDS/dToF"),
        ("power_motion_cny",       "动力系统",     0.10, "吸尘风机+驱动轮模组"),
        ("cleaning_module_cny",    "清洁模组",     0.15, "拖布/履带+泵+水箱"),
        ("battery_bms_cny",        "电池动力",     0.07, "电芯+BMS"),
        ("dock_system_cny",        "基站系统",     0.35, "加热+电解水+水路+集尘+触控"),
        ("structure_cmf_cny",      "机身结构与CMF",0.10, "外壳+注塑+喷涂+滚刷"),
        ("packaging_consumables_cny", "包装与耗材", 0.05, "尘袋+滤网+包材"),
    ]

    # 用户 overrides 中提取总 BOM 率或直接覆盖
    ov = overrides or {}
    # 估算总 BOM（旗舰机约 BOM率 46~58%）
    bom_rate = 0.52  # 默认中点
...
CLIENT_TOOLS: list[dict] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "生成 BOM 成本预估表，自动写回产品的 bom_cost 字段。\n"
            "overrides 可传入已知成本，如 {\"perception_control_cny\": 450}。"
        ),
        "input_schema": {
            "type": "object",
            "properties": {
...
现有拆机实测数据产品：科沃斯X2pro、石头P10pro、追觅X30pro、云鲸J4
...
添加新产品（完整6步流程）：
  • [bold]帮我添加 追觅X40 Ultra，按标准流程完成技术拆解和BOM预估[/bold]
  • 帮我添加 科沃斯T30S Max

成本分析：
  • 生成 云鲸逍遥002Max 的7桶BOM成本预估
  • 对比四款产品的 BOM 率和毛利率
  • 列出基站系统的标准件及成本区间
