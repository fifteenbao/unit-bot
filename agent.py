...
def tool_generate_bom_estimate(
    ...
    # 按市场定位调整 BOM 率（基于上市公司财报校验）
    # 参考：石头科技 2023 年报毛利率 52-54%，BOM 率约 46-48%
    segment = (entry.get("market_segment") or "").lower()
    if "旗舰" in segment or price >= 4000:
        bom_rate = 0.48    # 旗舰机 BOM 率 48%，毛利率 52%
    elif "入门" in segment or price < 2000:
        bom_rate = 0.58    # 入门机 BOM 率 58%，毛利率 42%
    else:
        bom_rate = 0.52    # 中高端 BOM 率 52%，毛利率 48%

    total_est = round(price * bom_rate)

    # 8 桶行业基准占比（基于零售价，各桶之和 = bom_rate）
    # 参考：石头科技财报 + 行业拆机数据校验
    if bom_rate == 0.48:  # 旗舰机（BOM 率 48%）
        # 占比为 BOM 总额的百分比（总和=100%）
        BUCKETS = [
            ("compute_electronics_cny", "算力与电子",   0.115, "SoC 主板·MCU·Wi-Fi·被动元件"),  # 5.5% of retail
            ("perception_cny",          "感知系统",     0.135, "LDS/dToF·结构光摄像头·IMU·超声波"),  # 6.5% of retail
            ("power_motion_cny",        "动力与驱动",   0.104, "吸尘风机·驱动轮电机·底盘升降"),  # 5.0% of retail
            ("cleaning_cny",            "清洁功能",     0.146, "拖布驱动·水泵·水箱·边刷·滚刷"),  # 7.0% of retail
            ("dock_station_cny",        "基站系统",     0.229, "集尘·水路·加热·基站电控·基站结构"),  # 11.0% of retail
            ("energy_cny",              "能源系统",     0.104, "电芯·BMS·充电 IC"),  # 5.0% of retail
            ("structure_cmf_cny",       "整机结构 CMF",  0.115, "外壳注塑·喷涂·模具摊销"),  # 5.5% of retail
            ("mva_software_cny",        "MVA+ 软件授权", 0.052, "组装人工·算法版税·OS·包材"),  # 2.5% of retail
        ]
    elif bom_rate == 0.58:  # 入门机（BOM 率 58%）
        # 占比为 BOM 总额的百分比（总和=100%）
        BUCKETS = [
            ("compute_electronics_cny", "算力与电子",   0.121, "SoC 主板·MCU·Wi-Fi·被动元件"),  # 7.0% of retail
            ("perception_cny",          "感知系统",     0.138, "LDS/dToF·结构光摄像头·IMU·超声波"),  # 8.0% of retail
            ("power_motion_cny",        "动力与驱动",   0.112, "吸尘风机·驱动轮电机·底盘升降"),  # 6.5% of retail
            ("cleaning_cny",            "清洁功能",     0.147, "拖布驱动·水泵·水箱·边刷·滚刷"),  # 8.5% of retail
            ("dock_station_cny",        "基站系统",     0.121, "集尘·水路·加热·基站电控·基站结构"),  # 7.0% of retail
            ("energy_cny",              "能源系统",     0.112, "电芯·BMS·充电 IC"),  # 6.5% of retail
            ("structure_cmf_cny",       "整机结构 CMF",  0.121, "外壳注塑·喷涂·模具摊销"),  # 7.0% of retail
            ("mva_software_cny",        "MVA+ 软件授权", 0.128, "组装人工·算法版税·OS·包材"),  # 7.5% of retail
        ]
    else:  # 中高端（BOM 率 52%）
        # 占比为 BOM 总额的百分比（总和=100%）
        BUCKETS = [
            ("compute_electronics_cny", "算力与电子",   0.115, "SoC 主板·MCU·Wi-Fi·被动元件"),  # 6.0% of retail
            ("perception_cny",          "感知系统",     0.135, "LDS/dToF·结构光摄像头·IMU·超声波"),  # 7.0% of retail
            ("power_motion_cny",        "动力与驱动",   0.106, "吸尘风机·驱动轮电机·底盘升降"),  # 5.5% of retail
            ("cleaning_cny",            "清洁功能",     0.144, "拖布驱动·水泵·水箱·边刷·滚刷"),  # 7.5% of retail
            ("dock_station_cny",        "基站系统",     0.173, "集尘·水路·加热·基站电控·基站结构"),  # 9.0% of retail
            ("energy_cny",              "能源系统",     0.106, "电芯·BMS·充电 IC"),  # 5.5% of retail
            ("structure_cmf_cny",       "整机结构 CMF",  0.115, "外壳注塑·喷涂·模具摊销"),  # 6.0% of retail
            ("mva_software_cny",        "MVA+ 软件授权", 0.106, "组装人工·算法版税·OS·包材"),  # 5.5% of retail
        ]
    ...
    for field, label, ratio, components in BUCKETS:
        known = (overrides or {}).get(field) or entry.get("bom_cost", {}).get(field)
        if known:
            mid = known
            lo = round(known * 0.9)
            hi = round(known * 1.1)
        ...
        rows.append({
            ...
            "数据来源": "已知" if ((overrides or {}).get(field) or entry.get("bom_cost", {}).get(field)) else "estimate",
        })
        bom_patch[field] = mid
...
CLIENT_TOOLS: list[dict] = [
    ...
    },
    {
        ...
        "description": (
            "为指定机型运行完整 4-Stage 拆机 Pipeline，生成 data/teardowns/{model}_teardown.csv：\n"
            "Stage 1 多源调研（FCC ID.io / MyFixGuide / 知乎 / 蓝牙SIG）\n"
            "Stage 2 SoC 伴随件推导（PMIC / RAM / ROM / AI授权）\n"
            "Stage 3 标准件库基准定价 + LCSC/1688 网络补全\n"
            "Stage 4 8桶汇总 & ±5% 偏差告警\n"
...
**优先执行 fcc_hint**（若非空）：
- 用 `web_fetch` 抓取品牌设备列表页（https://fccid.io/{grantee_code}），找到最相近型号
- 进入该型号详情，抓取 Internal Photos 和 Block Diagram 页面
- 从照片识别 PCB 芯片型号（SoC/MCU/Wi-Fi/PMIC 等），写入 `pcb_components`，`bom_source` 标注 `fcc`
- `fcc_hint` 为空（品牌未收录）时跳过，PCB 桶标注 `estimate`

**fcc_hint 完成后**，再执行 `web_search` 补全规格层（吸力 / 续航 / 越障 / 功能布尔值 / 价格 / 上市时间）。

### Step 3 — 写入数据库
调用 `save_product` 持久化写入 `data/products_db.json`，标注：
...
### Step 5 — BOM 估算（8桶）
调用 `generate_bom_estimate`，输出 8桶结构成本预估表：
