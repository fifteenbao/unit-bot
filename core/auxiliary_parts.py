...
def estimate_auxiliary_cost(bom_bucket: str, part_count: int = 0) -> dict[str, Any]:
    ...
    """
    bucket_multiplier = {
        "structure_cmf":       1.4,   # 结构件桶辅助件多
        "cleaning":            1.2,   # 清洁模块水路、密封件多
        "power_motion":        1.1,
        "dock_station":        1.3,   # 基站系统外壳+水路辅助件多
    }.get(bom_bucket, 1.0)
    ...
    for aux_type, price in AUX_PRICE.items():
        qty = int(DEFAULT_AUX_QTY[aux_type] * bucket_multiplier)
        cost = price * qty
        total_aux_cost += cost
        total_aux_parts += qty
    ...
    return {
        ...
        "suggestion": "建议通过卡扣替代螺丝（tiny类），可降低辅助零件数约15-25%",
    }
