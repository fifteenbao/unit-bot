...
KEYWORD_RULES: list[tuple[str, str, str, str]] = [
    ...
    (r"霍尔开关",                          "perception", "霍尔开关", "霍尔开关"),
    (r"霍尔检测|霍尔板",                    "perception", "霍尔开关", "霍尔检测板"),
    ...
    (r"小五金散件|五金散件|辅料散件",            "structure_cmf", "小五金散件", "小五金散件(聚合)"),
    # ── C33 实测补充规则（is_aux 因前缀否决而漏掉的件）──────────────────
    # 线束类 (主板线/检测线 → 算力&电子)
    (r"主板.*(电源线|连接线)|^主板.*线$",        "compute_electronics", "板间线束", "主板线束"),
    (r"^LDS.*(连接线|线|线缆)",                   "compute_electronics", "板间线束", "LDS 线束"),
    (r".{0,6}检测线$|.{0,6}在位线$",              "compute_electronics", "板间线束", "板间检测线"),
    # 基站水路/气路 (从 主机/基站 区域接出的硅胶管)
    (r"^(加|出|进)?(清水|污水).*(大管|小管|短管|长管|连接管)|散热.*铜管.*管",
     "dock_station", "基站水路杂件", "基站水管(聚合)"),
    (r"^气泵.*(气管|进气|出气|吸气)|气路.*管",   "dock_station", "基站水路杂件", "基站气路(聚合)"),
    # 碰撞传感的结构件 (碰撞左右/软胶/前撞弹片/扭簧)
    (r"^碰撞[左右](-[A-Z])?$|^碰撞软胶$|^前撞.*(弹片|簧)|碰撞.*扭簧",
     "perception", "碰撞传感组件", "碰撞结构件(聚合)"),
    # 回充结构 (后充电支架)
    (r"后充电支架",                              "dock_station", "充电弹片", "后充电支架"),
    # 出水滋水条 (湿拖辅助)
    (r"滋水条|出水条|滋清水入口.*连接管",         "cleaning", "清水盒", "滋水条/入口管"),
    # 减震/防水泡棉 (装在电机/电池仓上)
    (r"(电池仓|电机|主板).*(泡棉|减震|防水)|减震泡棉|防水泡棉",
     "structure_cmf", "小五金散件", "减震/防水泡棉(聚合)"),
    # 清洁附件 (随机赠送的清洁刷等)
    (r"清洁小工具|清洁刷",                        "structure_cmf", "小五金散件", "随机清洁工具"),
