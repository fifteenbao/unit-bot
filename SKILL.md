...
```yaml
feishu:
  # obj_token 即多维表格的 app_token，通过 wiki API 响应的 obj_token 字段获取
  # 用 scripts/resolve_wiki_token.py 脚本一键解析
  product_obj_token: ""
  teardown_obj_token: ""
...
> 飞书链接未配置时，同步操作静默跳过，本地 `data/` 数据不受影响。
...
|------|------|------|
| 产品数据库（飞书产品表 / 本地 JSON） | `database` | 规格 / 价格 / 功能，人工维护，置信度最高 |
