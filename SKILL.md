...
| FCC 上游 | `data/teardowns/fcc/{slug}/{slug}_fcc_{date}.csv` | `fetch_fcc.py` 产出，Stage 1 优先上游 |
...
| 2 | 型号别名解析 | `core.model_aliases` | `data/products/model_aliases.json` |
...
| 命令 | 用途 | 等价自然语言 |
|------|------|------------|
| `/bom <品牌> <型号>` | BOM 完整分析（核心命令） | 〔品牌〕〔型号〕，分析 BOM 成本 |
| `/parts <关键词>` | 零部件跨机型查询 | 用了哪些〔关键词〕？ |
| `/cut <品牌> <型号>` | 降本机会识别 | 〔型号〕哪些件是溢价件，降本空间在哪？ |
| `/vs <型号A> vs <型号B> [--bucket <桶>]` | 子系统/整机对标 | 对比〔A〕和〔B〕的〔桶〕成本 |
| `/find <关键词\|桶>` | 数据库直查 | 列出拆机数据中的〔关键词〕 |
| `/framework` | 导出 8 桶对账 CSV | 导出 8 桶框架对账表 |
...
### `/vs <型号A> vs <型号B> [--bucket <桶>]` — 子系统对标
> `/vs 科沃斯 X8 Pro vs 石头 S8 MaxV Ultra --bucket dock_station`
→ 按指定桶 `typical_items` 逐项对比；不带 `--bucket` 则整机 8 桶对比

### `/find <关键词|桶>` — 数据库直查
> `/find CPU`（或 `/find compute_electronics`）
→ 遍历 `data/teardowns/*.csv` 抽取对应桶/关键词
...
| `data/products/model_aliases.csv` | 国内/海外型号映射 | 新机型发布时补映射 |
