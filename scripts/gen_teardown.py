...
职责：爬取元器件型号 + 判断置信度 + 对照 7 桶框架审计覆盖与成本偏差
价格来源：components_lib.csv（人工维护，权威来源）→ standard_parts.json（基准 fallback）
            不调用 API 查价；定价更新请维护 data/lib/components_lib.csv

7 桶标准模板：core/bom_8bucket_framework.json（通用、无敏感数据）
  - 7桶全部属于一级「硬件物料」，其余4个一级大类(人工+机器折旧/销售+管理费用/研发均摊/仓储物流成本)不纳入本框架
  - 桶定义、典型子项、行业占比基准、归桶边界、user_value_weight、dfma_levers 全部来自该文件
  - 修改模板后本脚本自动生效，无需改代码

职责边界：
  - 本脚本只产出"是什么成本"——7 桶占比、BOM/MSRP、偏差告警
...
_DISCOVERY_SYSTEM = (
    ...
    "【4级拆机成本结构】：\n"
    "一级·成本大类(5项)：硬件物料、人工+机器折旧、销售+管理费用、研发均摊、仓储物流成本\n"
    "二级·7桶(全部归入硬件物料)：按下方7桶框架逐桶拆解\n"
    "三级·功能模块：每桶按物理功能分为若干模块(如清洁桶→恒压活水洗地/高密度滚刷/活水循环/精密机械结构)\n"
    "四级·组件：最小可计价单元，对应 BOM 单行\n"
...
**7个BOM桶说明**（来自 core/bom_8bucket_framework.json，标准模板。7桶全部属于一级「硬件物料」）
...
2. **基站系统** `dock_station` 按三级功能拆为三大模块（若带基站）：
   ...
   - **水路系统**: 清水桶、污水桶、清洗盘+刮条、水泵、电磁阀、PTC烘干（按实际功能取舍）
   - **电气系统**: 电源板、集尘风机、充电弹片+回充组件、子PCBA、AC电源线（按实际功能取舍）

3. **算力桶** `compute_electronics` 必须至少包含：
   - 核心算力模块：主控 SoC（哪怕只知道厂商、型号填"未确认"也要列出）
   - 主板与互联模块：主板 PCB + 无线模组（Wi-Fi/BT）

4. **清洁桶** `cleaning` 若该机型带拖地，必须包含：
   - 恒压活水洗地模块：拖布本体（双转盘/滚筒/履带三选一）
   - 活水循环模块：清水泵 + 主机水箱（清水/污水）

5. **bom_bucket 字段**必须严格使用 7 个合法 key（见上方桶说明），不要自创命名。
   - 注意：组装人工、SLAM版税、包装材料、物流运保等不属于7桶框架，不要列为 BOM 行。

6. **传感器方案分档**（影响 perception 桶的成本与占比）— 根据调研信息判断产品属于哪一档：
   - **低配·纯惯导**: 仅 IMU + 轮速计 + 碰撞 + 地检。无激光雷达/ToF/视觉。perception 桶成本极低 (¥20-80, 占 BOM 2-5%)。
   - **中配·激光雷达**: LDS + 沿墙线激光 + 超声波 + IMU。结构化建图+路径规划。perception 桶成本中等 (¥150-350, 占 BOM 8-14%)。
   - **高配·视觉 AI**: LDS + 3D 结构光/dToF + RGB 视觉 + AI 加速器(NPU) + 多传感器融合。perception 桶成本高 (¥300-600, 占 BOM 12-25%)。
   在 perception 桶的 section 字段中标注对应的传感器档位模块名 (如"核心导航"/"前视避障"/"视觉AI")。

6. **name 字段**优先使用 framework `typical_items` 的**完整名称**，section 字段使用【】标注的模块名，便于下游匹配标准件库。

**目标**：覆盖全部 7 个桶，总行数 ≥ 25（入门机）/ 35（中端）/ 45（旗舰带基站）。\
"""
...
def stage1_discovery(model: str, msrp: float,
    ...
    if not rows:
        ...
        print(
            f"  ⚠ Stage 1 LLM 返回空数组 [] — 未找到 {model!r} 的公开拆机资料。\n"
            f"    原始响应已存: {raw_path}\n"
            f"    → 降级为纯框架估算: Stage 4 将按 typical_items 生成基线 BOM (全部标记 confidence=estimate)。\n"
            f"    如需更准确的结果, 尝试换机型名或手工填 CSV 后用 --csv 加载。"
        )
        return []
...
# ══════════════════════════════════════════════════════════════════
#  Stage 4 — Aggregate & Bias Audit（7 桶金额汇总 + ±5% 偏差告警）
# ══════════════════════════════════════════════════════════════════
...
def stage4_aggregate_audit(
    ...
    # ── 辅助物料估算 (AUX) ──────────────────────────────────────────
    # 对每桶按 DFMA 辅料标准估算未在 BOM 中逐项列出的小零件成本
    aux_by_bucket: dict[str, dict] = {}
    aux_grand = 0.0
    aux_parts_total = 0
    dfa_score_penalty = 0.0
    for bkt, _ in BUCKETS:
        aux_result = estimate_auxiliary_cost(bkt, bucket_counts[bkt])
        aux_by_bucket[bkt] = aux_result
        aux_grand += aux_result["aux_cost"]
        aux_parts_total += aux_result["aux_part_count"]
        # DFA Score 惩罚: 每桶辅助件装配难度加权
        dfa_score_penalty += aux_result["assembly_difficulty_increase"]
    ...
    print(f"\n  [Stage 4] 7桶金额汇总 (查价: components_lib → standard_parts → 兜底)")
    ...
    print(f"  {'辅助物料估算':16s} {aux_parts_total:>4d}  {aux_grand:>10.2f}  "
          f"{'—':>7s}  {'—':>8s}  (DFMA 辅料)")
    print(f"  {'BOM + 辅料合计':16s} {'':>4s}  {grand_with_aux:>10.2f}")
    ...
    # DFA Score (装配复杂度, 100分制)
    print(f"\n  DFA Score: {dfa_score}/100  "
          f"(辅料装配难度扣分: {dfa_score_penalty:.1f}, "
          f"估算辅料零件 +{aux_parts_total} 件)")
    for bkt, _ in BUCKETS:
        aux = aux_by_bucket[bkt]
        if aux["aux_cost"] > 1.0:  # 只打印象征性的桶
            print(f"    {BUCKET_MAP.get(bkt, bkt):16s} 辅料 +¥{aux['aux_cost']:.2f}  "
                  f"+{aux['aux_part_count']}件  "
                  f"难度+{aux['assembly_difficulty_increase']:.1f}")
    ...
    if bias_alerts:
        ...
        print(f"\n  ✓ 7 桶占比全部在基准 ±{tolerance:.0f}% 内")

    all_bias = bias_alerts + ([ratio_alert] if ratio_alert else [])
    return {
        "grand_total": round(grand, 2),
        "grand_total_with_aux": round(grand_with_aux, 2),
        "aux_cost": round(aux_grand, 2),
        "aux_part_count": aux_parts_total,
        "dfa_score": dfa_score,
        "aux_by_bucket": {k: {
            "cost": v["aux_cost"], "parts": v["aux_part_count"],
            "difficulty": v["assembly_difficulty_increase"],
        } for k, v in aux_by_bucket.items()},
        "buckets_money": bucket_money,
        "bias_alerts": all_bias,
        "level1_costs": l1_costs if l1_costs else {},
        "estimated_total_cost": round(l1_total, 0) if l1_total else None,
    }
    ...
    if existing_csv and existing_csv.exists():
        ...
        if auto_csv.exists():
            ...
            if not rows:
                ...
                if existing:
                    rows = load_csv(existing[-1])
                    print(f"  ✓ Stage 1 无结果，回退至已有 CSV: {existing[-1].name}（{len(rows)} 条）")
                else:
                    print(f"  → Stage 1 无结果且无历史 CSV，Stage 4 framework_fill 将生成纯框架估算 BOM")
